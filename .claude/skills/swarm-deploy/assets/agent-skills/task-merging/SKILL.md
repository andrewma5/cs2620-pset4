...
4. **Run sanity tests. On failure, revert and hand off a fix.**
   ```bash
   if [ -d tests ] || [ -f pytest.ini ] || [ -f pyproject.toml ]; then
       if ! pytest -q; then
           FAILING=$(pytest -q 2>&1 | grep -E '^FAILED' | head -3 | tr '\n' '; ')
           echo "[merging] post-merge tests failed; reverting and handing off fix"
           git reset --hard HEAD~1
           tm /main_lock_release "$(jq -n --arg aid "$AGENT_ID" --argjson s $((RANDOM*RANDOM)) \
               --argjson lt "$LOCK_TOK" \
               '{agent_id:$aid,serial:$s,lock_token:$lt}')"
           NEW_CHILDREN_JSON=$(jq -n --arg sha "$IMPL_SHA" --arg failing "$FAILING" \
               '[{
                 type: "implement",
                 title: ("fix tests broken by " + $sha),
                 prompt: ("Branch " + $sha + " breaks tests when merged into main: " + $failing + ". Fix the tests or the branch, then this work will be re-merged."),
                 branch_base: "main"
               }]')
           tm /task_complete "$(jq -n \
               --arg aid "$AGENT_ID" --argjson s $((RANDOM*RANDOM)) \
               --arg id "$TASK_ID" --argjson tok "$TOK" \
               --arg sum "post-merge tests failed: $FAILING; handing off fix" \
               --arg br "" \
               --argjson kids "$NEW_CHILDREN_JSON" \
               '{agent_id:$aid,serial:$s,id:$id,token:$tok,
                 result_summary:$sum,result_branch:$br,new_children:$kids}')"
...
- The required tools (`git`, `pytest`) are missing from the
  environment.
